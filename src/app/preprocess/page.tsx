// src/app/preprocess/page.tsx - Trang 1: Tiền xử lý Data (gọi API /preprocess)
'use client';
...
export default function PreprocessPage() {
  ...
  const t = {
    ...
    description: language === 'vi' ? 'Tải và xử lý dữ liệu thiên văn từ API backend.' : 'Load and process astronomical data from backend API.',
    button: language === 'vi' ? 'Xử lý Dữ liệu' : 'Process Data',
    ...
    result: language === 'vi' ? 'Kết quả xử lý:' : 'Processing result:'
  };
  ...
  const handleProcess = async () => {
    ...
    setError(null);
    try {
      // Gọi API Flask backend (giả sử chạy ở http://localhost:5000/preprocess)
      const response = await fetch('http://localhost:5000/preprocess', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'preprocess_exoplanet_data' }) // Payload mẫu
      });
